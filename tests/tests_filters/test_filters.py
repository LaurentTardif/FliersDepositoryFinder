...
class TestFilters(unittest.TestCase):
        ...
        try:
            ...
            for row in generated_content:
                ...
                if nom in entrees_deja_filtrees:
                    ...
                    self.assertEqual(row.get('Filtré', ''), 'OUI',
                                   f"{nom} devrait rester filtré")

                    # Vérifier que la raison a été mise à jour avec la règle automatique
                    raison_actuelle = row.get('Raison du filtre', '')
                    self.assertEqual(raison_actuelle, attendu['nouvelle_raison'],
                                   f"{nom}: {attendu['description']}")
    ...
    def test_cas_positifs_non_filtres(self):
        ...
        try:
            ...
            cas_positifs = {
                ...
                'Boulangerie Bien Notée': {
                    'attendu': 'NON',
                    'raison': 'Boulangerie avec ≥50 avis (règle 1 non applicable)'
                },
                'Garage Ouvert': {
                    'attendu': 'NON',
            ...
            for row in generated_content:
                ...
                if nom in cas_positifs:
                    ...
                    self.assertEqual(filtre_status, attendu,
                                   f"{nom} ne devrait PAS être filtré. {raison}")

                    # Vérifier aussi que la raison est "Pas de filtre"
                    raison_filtrage = row.get('Raison du filtre', '')
                    self.assertEqual(raison_filtrage, 'Pas de filtre',
                                   f"{nom} devrait avoir 'Pas de filtre' comme raison")

            # Compter les cas positifs trouvés
            cas_positifs_trouves = sum(1 for row in generated_content
                                     if row.get('Nom', '') in cas_positifs and row.get('Filtré', '') == 'NON')

            self.assertEqual(cas_positifs_trouves, len(cas_positifs),
    ...
    def test_couverture_filtres_multiples(self):
        ...
        try:
            result = subprocess.run([
                'python', str(self.script),
                str(input_file), temp_output_path, '--verbose'
            ], capture_output=True, text=True, encoding='utf-8')
            ...
            self.assertGreaterEqual(raisons_comptees.get('avis_insuffisant', 0), 2,
                                  "Il devrait y avoir au moins 2 entreprises filtrées pour 'Nombre d'avis insuffisant'")
            self.assertGreaterEqual(raisons_comptees.get('jours_fermeture', 0), 2,
                                  "Il devrait y avoir au moins 2 entreprises filtrées pour 'Jours de fermeture'")
            self.assertGreaterEqual(raisons_comptees.get('restaurant_note', 0), 2,
                                  "Il devrait y avoir au moins 2 restaurants filtrés pour 'Note insuffisante'")
            self.assertGreaterEqual(raisons_comptees.get('coiffeur_note', 0), 1,
                                  "Il devrait y avoir au moins 1 coiffeur filtré pour 'Note insuffisante'")

            # Vérifier qu'on a des cas positifs (non filtrés)
            self.assertGreaterEqual(cas_non_filtres, 4,
                                  "Il devrait y avoir au moins 4 cas positifs (non filtrés)")
