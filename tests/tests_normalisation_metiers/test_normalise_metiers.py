...
class TestNormaliseMetiers(unittest.TestCase):
    ...
    def test_normalisation_complete(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["python", str(self.normalise_script), str(self.input_file), temp_output_path, str(self.reference_file)],
                capture_output=True,
                text=True,
                encoding="utf-8",
            )
            ...
            for i, (generated_row, expected_row) in enumerate(zip(generated_content, expected_content)):
                with self.subTest(ligne=i + 1):
                    self.assertEqual(generated_row["Nom"], expected_row["Nom"], f"Ligne {i+1}: Nom différent")
                    self.assertEqual(generated_row["Adresse"], expected_row["Adresse"], f"Ligne {i+1}: Adresse différente")
                    self.assertEqual(generated_row["Ville"], expected_row["Ville"], f"Ligne {i+1}: Ville différente")
                    self.assertEqual(
                        generated_row["Metier_normalise"],
                        expected_row["Metier_normalise"],
    ...
    def test_gestion_metiers_inconnus(self):
        ...
        try:
            # Exécution du script
            result = subprocess.run(
                ["python", str(self.normalise_script), str(input_file), temp_output_path, str(self.reference_file)],
                capture_output=True,
                text=True,
                encoding="utf-8",
            )
